...
When the user asks for a batch, demo library, or says intermediate files are not needed, produce a final upload bundle instead of forcing every per-App intermediate JSON into the handoff. Required final files:

- MPK files for every App.
- PNG/JPEG/WebP screenshots for every App.
- `upystore_upload_manifest.json` with manifest metadata, MPK sha256/size, icon path, screenshot path, release notes, hardware tags, and manual upload guidance.
- `artifact_manifest.json` when a browser/frontend needs to display downloadable artifacts.
...
PYTHONDONTWRITEBYTECODE=1 /home/leeqingshui/mp_env/bin/python \
  ...
  --mpk-dir <repo-root>/tmp/<batch-name>/mpk \
  --screenshot-dir <repo-root>/tmp/<batch-name>/screenshots \
  --output <repo-root>/tmp/<batch-name>/upystore_upload_manifest.json \
  --artifact-manifest-output <repo-root>/tmp/<batch-name>/artifact_manifest.json
```

Do not mark the batch as ready for upystore upload if any App is missing `publisher`, MPK, icon, or publish-ready screenshot. A batch upload manifest is a final result artifact, not a replacement for per-App `publish_result.json` when the user requests a full single-App release workflow.
