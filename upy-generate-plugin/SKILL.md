...
| 生成 LDR/photoresistor/GL5516 光敏阈值逻辑前 | `knowledge/ldr_photoresistor_polarity.pitfall.json` |
...
3. 在写 `generate_plan.json`、Mock、业务框架或业务代码之前，必须对 `manifest_content.devices[]` 中显式的 driver workflow status 执行 gate。`driver.status`、`driver.driver_status` 或 `device.driver_status` 为 `cold_driver_required`、`pending_validation`、`unverified`、`failed` 或未知值时，必须输出 `partial`、`next_phase=null`、`structured_errors[]`，并引导用户先运行 `upy-gen-driver-plugin` pipeline mode 生成 `firmware/drivers/<driver_id>_driver/`。显式 `ready` 只有同时带 `driver.path` 与 `hardware_marker=SELF_TEST_PASS:<driver_id>:<scenario>` 才能通过；普通 `builtin_runtime`、`micropython_lib`、`upypi`、`awesome-micropython`、`github`、`none`、`manual` source-only 依赖若未写 workflow status，继续走既有依赖解析。MicroPython builtin modules such as `dht` may stay on this simple runtime path; writing a wrapper, mock, or device test does not by itself trigger driver_ready. Only an explicit `driver.status` / `driver.driver_status` / `device.driver_status` enters the driver workflow contract, so do not claim `driver_status=ready` unless the manifest can satisfy the ready contract.
4. 运行前询问用户是否补充装置行为。只允许补充业务行为、阈值、周期、状态机、日志和模拟场景；新增硬件或改引脚必须回退到 analyze/select-hw/scaffold。
5. 写 `project/generate_plan.json`，先只规划不写运行时代码。计划必须包含 scheduler_mode、driver adapters、tasks、config_constants、main_assembly、tests、resource_plan、cloud_integrations（如果需要云 API）。语音、传感器、云 API、状态机、pipeline 或跨 tick 业务流还必须包含 `data_flow_contract[]`。每条 contract 必须包含 `name`、`producer`、`consumer`、`invariant`；跨 tick/跨状态/跨任务时还必须包含 `storage`。测试覆盖必须使用 `covered_by_tests`（优先，非空字符串数组，例如 `["test/pc/test_flow.py::TestFlow::test_contract"]`）或 `test_path`（单个非空测试路径字符串）。不要发明 `tests`、`coverage`、`contract_tests`、`test_coverage` 等 checker 不接受的字段。简单 GPIO blink、固定周期执行器输出、单 task 直接控制不需要 `data_flow_contract[]`。随后运行 `scripts/check_generate_plan.py --project-dir <project_root> --require-plan`，失败则停在 partial，不要继续大规模写代码。
...
- Do not hand-author `project/tools/flash_device.py` or `project/tools/read_device_log.py` during generate. They are scaffold-owned helpers rendered by `apply_scaffold`; if missing or incompatible, restore scaffold output or set `next_phase=null` with `next_phase_decision` for code-only/manual-flash delivery.
...
- For LDR/photoresistor/GL5516 light sensing, read `knowledge/ldr_photoresistor_polarity.pitfall.json` before generating threshold logic. Prefer calibration APIs over raw ADC comparisons. If raw ADC thresholds are used, generated code/config must name the polarity assumption; do not assume raw ADC rises with brightness.
...
- MicroPython 内置模块（例如 `dht`）可以作为 `builtin_runtime` 简单路径直接使用；缺少显式 `driver_status` 时不要进入 driver_ready。生成 wrapper/mock/device test 是允许的工程增强，但不会自动触发 driver_ready；只有显式声明 `driver.status`、`driver.driver_status` 或 `device.driver_status` 才进入该合同。
