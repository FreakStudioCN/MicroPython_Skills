...
| # | 改写项 | 规范来源 |
|---|---|---|
| 1 | 补全文件头 7 行注释，`# @License : MIT` 独立成行 | 二 |
| 2 | 补全 `__version__`、`__author__`、`__license__`、`__platform__` | 三 |
| 3 | 补全 6 个分区标注注释，顺序正确 | 四 |
| 4 | 所有 `raise`/`print` 字符串改为英文 | 五·规则3 |
| 5 | `__init__` 所有参数加类型注解，返回值注解 `-> None` | 五·规则7、六·6.6 |
| 6 | 每个有参数的方法加参数校验 + `raise` | 五·规则8、六·6.8 |
| 7 | 类和每个公共方法加中英双语 docstring（含 Args/Returns/Raises/Notes） | 六·6.4、6.7 |
| 8 | 行内注释改为中文 | 十六 |
| 9 | 公共方法返回值加类型注解 | 十九·15.1 |
...
| 16 | 函数命名遵循动词前缀约定（`read_`/`set_`/`get_`/`init_`/`reset_`/`_`私有） | 十八·14.3 |
| 17 | 多值返回改用 `tuple`，原始数据用 `bytearray` | 十八·14.4 |
| 18 | 通用功能（CRC、单位转换）抽离到类外 | 九·9.1 |
| 19 | 固定硬件参数和默认配置声明为类属性大写常量 | 九·9.3 |
| 20 | 配置属性通过 `set_xxx()`/`get_xxx()` 封装，禁止外部直接修改 | 九·9.8 |

**P2 — 可选（按实际代码硬件特性决定）**

| # | 改写项 | 适用场景 | 规范来源 |
|---|---|---|---|
| 21 | ISR 回调改用 `micropython.schedule` | 有中断回调 | 九·9.5、十八·14.2 |
| 22 | 高频读取改用 bytearray 复用缓冲区 | 有频繁 I/O | 四·4.1 |
| 23 | 添加 `__enter__`/`__exit__` 上下文管理器 | 需资源自动释放 | 九·9.7 |
| 24 | 平台兼容性适配（`sys.platform` 判断） | 多平台部署 | 九·9.9 |
| 25 | 数据防抖与缓存 | 高频采样传感器 | 九·9.10 |
| 26 | 单例模式（`_instance`/`get_instance()`） | 硬件资源唯一性 | 九·9.12 |
| 27 | 非阻塞采样改用 `machine.Timer` | 有 `time.sleep` 阻塞主循环 | 九·9.6 |
| 28 | 自定义异常类（`SensorError` 等） | 复杂错误场景 | 九·9.2 |
...
| 12 | 功能函数加简短 docstring | 十·10.4 |
| 13 | 全局变量命名遵循 `snake_case` | 十八·14.3 |

**P2 — 可选**

| # | 改写项 | 适用场景 |
|---|---|---|
| 14 | 批量操作封装为独立函数供 REPL 一键调用 | 有多个同类 API |
...
| 5 | 主程序区 `try/except/finally` + 资源清理 | 十·10.5 |
| 6 | `raise`/`print` 全英文，行内注释中文 | 五·规则3、十六 |
...
| 5 | 硬件要求 | 推荐硬件表格 + 引脚说明表格 | 十一·11.4 |
| 6 | 文件结构 | 文件树 | 十一·11.5 |
| 7 | 文件说明 | 按文件逐个解释用途 | 十一·11.5 |
| 8 | 快速开始 | 分步说明 + 代码示例 | 十一·11.6 |
| 9 | 注意事项 | 工作条件、使用限制、兼容性 | 十一·11.1 |
| 10 | 版本记录 | 表格：版本号、日期、作者、修改说明 | 十一·11.1 |
| 11 | 联系方式 | 邮箱或 GitHub | 十一·11.1 |
| 12 | 许可协议 | 区分官方模块 MIT 与自编驱动 CC BY-NC 4.0 | 十一·11.7 |
...
| 2 | `urls`：扫描驱动目录下所有 `.py` 文件，生成正确映射 | 十四·14.2 |
...
| 3 | 其他第三方依赖 | 查询 `https://upypi.net/api/search?q={依赖名}`，有结果用 upypi URL，无结果用 `github:` 格式并标注 ⚠️ 需手动确认 |
